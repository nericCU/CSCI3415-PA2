...
-- Here is the code for "Total Number of Collaborations and Collaborating Pairs"
  ...
  --Below is the average calculated with "Total Collaborations in all Comic Books" divided by "Total Characters"
    ...
    put("The average number of collaborators is ");-- & Float'Image (averageCollab) & "." );
...
-- Here is alternative method where average is: Total Collaborations / Comics with Collabs
  declare
    av1 : Float := Float(CollaborationsTotal);
    av2 : Float := Float(N_Characters);
    averageCollab : Float := av1 / av2;

	counter : Natural := 0;
	counter2 : Natural := 0;
    
  begin
    put("2The average number of collaborators is ");-- & Float'Image (averageCollab) & "." );
    put(averageCollab, Fore => 4, Aft => 2, Exp => 0); New_Line;
	...
	for I in Edges'Range(2) loop -- values 6487 through 19428.
		...
		for J in Edges'Range(1) loop   --values 1 - 6486
			...
			if J = 6486 then
				Put_Line( "Hello World" & Positive'Image(J) );
			end if;
		end loop;
	end loop;
		
			
	put("books with collab is " & Positive'Image(counter2) & ".");
